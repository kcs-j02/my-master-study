...
int main() {
  ...
  // Green Context組み込み
    vector<int> count_of_indegree_each_stream(stream_count, 0);
    int count_of_total_indegree = 0;

    tf.for_each_task([&](tf::Task t){
      auto &ni = node_info[t.name()];
      if (ni.stream_id >= 0 && ni.stream_id < stream_count) {
        count_of_indegree_each_stream[ni.stream_id] += ni.indeg;
      }
      count_of_total_indegree += ni.indeg;
    });
    
    for(int i = 0; i < stream_count; i++){
      cout << "count_of_indegree_each_stream[" << i << "] = "
      << count_of_indegree_each_stream[i] << endl;
    }
    cout << "count_of_total_indegree =" << count_of_total_indegree << endl; 
    
    cudaDeviceProp prop{};
    cudaGetDeviceProperties(&prop, 0);
    int sm_count = prop.multiProcessorCount;
    cout << "max SM count = " << sm_count << "\n";

    vector<int> count_of_SM(stream_count, 0);

    for(int i = 0; i< stream_count; i++){
      count_of_SM[i] = ((sm_count - 10) * count_of_indegree_each_stream[i])/count_of_total_indegree;
      cout << "count_of_SM[" << i <<"] = " << count_of_SM[i] << endl; 
    }
    
    br.green_ctx_ms = measure_ms([&]{
    ...
    for (int i = 0; i < stream_count; ++i) {
      ...
      cudaDevSmResourceSplit(result.data(), stream_count, &initial_GPU_SM_resources, nullptr, 0, group_params.data());

      vector<cudaDevResourceDesc_t> resource_desc(stream_count);

      for(int i= 0; i < stream_count; i++){
        cudaDevResourceGenerateDesc(&resource_desc[i], &result[0], 1);
      }
